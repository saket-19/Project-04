...
import in.co.rays.proj4.utility.EmailBuilder;
import in.co.rays.proj4.utility.EmailMessage;
import in.co.rays.proj4.utility.EmailUtility;
...
public class UserModel {
	...
	public long add(UserBean bean) throws DuplicateRecordException, SQLException, DatabaseException {
		Connection conn = null;
		int pk = 0;

		UserBean existbean = new UserBean();
		try {
			existbean = findByLogin(bean.getLogin());
		...
		try {
			pstmt = conn.prepareStatement("insert into st_user values(?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	public long registerUser(UserBean bean) throws DuplicateRecordException, ApplicationException, SQLException, DatabaseException {
